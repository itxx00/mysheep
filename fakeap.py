...
def iptables(inet_iface):
    ...
    os.system('iptables -t nat -A POSTROUTING -s 10.0.0.0/24 ! -d 10.0.0.0/24 -p tcp -j MASQUERADE --to-ports 1024-65535')
    os.system('iptables -t nat -A POSTROUTING -s 10.0.0.0/24 ! -d 10.0.0.0/24 -p udp -j MASQUERADE --to-ports 1024-65535')
    os.system('iptables -t nat -A POSTROUTING -s 10.0.0.0/24 ! -d 10.0.0.0/24 -j MASQUERADE')
...
def main(args):
    ...
    print "Found internet iface: %s" % inet_iface
    ap_iface = AP_iface(interfaces, inet_iface)
    if not ap_iface:
        sys.exit('['+R+'-'+W+'] Found internet connected interface in '+T+inet_iface+W+'. Please bring up a wireless iface to use as the fake ap.')
    print "Found ap iface: %s" % ap_iface
    ...
    while 1:
        ...
        proc = Popen(['cat', '/var/lib/dhcpd/dhcpd.leases'], stdout=PIPE, stderr=DN)
        for line in proc.communicate()[0].split('\n'):
            print line
        time.sleep(1)
